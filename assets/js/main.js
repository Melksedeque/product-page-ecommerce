...
cartButton.addEventListener('click', () => {
    ...
    const existingItem = document.querySelector(`[data-id="${uniqueId}"]`)
    if (existingItem) {
        ...
        const formattedTotalPrice = `<strong>$${totalPrice.toFixed(2)}</strong>`
        
        itemTitle.innerHTML = `${productName}<br>${productCurrentPrice} x ${productQuantity} ${formattedTotalPrice}`
